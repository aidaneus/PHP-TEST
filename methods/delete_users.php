...
class deleteUsers{
    ...
    function check_right($rows, $i, $delete){
        if ($delete->rights[$i] == 'send_messages' && $rows[$delete->rights[$i]] != $delete->send_messages && $delete->send_messages != 'block')
            $delete->send_messages = $rows[$delete->rights[$i]];
        if ($delete->rights[$i] == 'service_api' && $rows[$delete->rights[$i]] != $delete->service_api && $delete->service_api != 'block')
            $delete->service_api = $rows[$delete->rights[$i]];
        if ($delete->rights[$i] == 'debug' && $rows[$delete->rights[$i]] != $delete->debug && $delete->debug != 'block')
